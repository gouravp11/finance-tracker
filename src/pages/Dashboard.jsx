    ...
    return (
        <div className="relative" ref={ref}>
            ...
            >
                ...
                <svg className="ml-2 h-4 w-4 text-gray-500 dark:text-gray-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            ...
            {open && (
                <ul className="absolute left-0 right-0 top-full mt-1 z-20 bg-white dark:bg-slate-700 border dark:border-slate-600 rounded shadow max-h-64 overflow-y-auto">
                    {options.map((opt) => (
                        <li key={opt} onClick={() => { onSelect(opt); setOpen(false); }} className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-slate-600 cursor-pointer text-sm dark:text-white">
                            {opt}
                        </li>
                    ))}
                </ul>
...
export default function Dashboard() {
    ...
    const totalExpenses = transactions.reduce((acc, t) => t.type === "expense" ? acc + (Number(t.amount) || 0) : acc, 0);
    ...
    const hasActiveFilters = searchTerm || filterType !== "all" || filterCategory || filterStartDate || filterEndDate;

    const clearFilters = () => {
        setSearchTerm(""); setFilterType("all"); setFilterCategory(""); setFilterStartDate(""); setFilterEndDate("");
    };
    ...
    const filterPanel = (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                ...
                {hasActiveFilters && (
                    <button onClick={clearFilters} className="text-xs text-blue-600 dark:text-blue-400 underline">Clear</button>
                )}
            </div>
            <div>
            ...
            <div>
                ...
                <FilterDropdown options={categoryOptions} value={filterCategory || "All categories"}
                    onSelect={(val) => setFilterCategory(val === "All categories" ? "" : val)} placeholder="All categories" />
            </div>
            <div>
    ...
    return (
        <div className="min-h-screen">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6 bg-white dark:bg-slate-800 border-b dark:border-slate-700">
                <h2 className="text-xl sm:text-2xl font-semibold mb-1 dark:text-white">Dashboard</h2>
                <p className="text-sm text-gray-700 dark:text-gray-300">Welcome back, {user.displayName || user.email}.</p>
                <p className="mt-1 text-xs sm:text-sm text-gray-500 dark:text-gray-400">Manage your finances with powerful filtering and analytics.</p>
            </div>
            ...
            <div className="lg:hidden max-w-7xl mx-auto px-4 py-3">
                <div className="grid grid-cols-3 gap-2">
                    <div className="bg-white dark:bg-slate-800 rounded shadow p-3 text-center">
                        ...
                        <p className="text-sm font-bold text-green-600 dark:text-green-400">${totalIncome.toFixed(0)}</p>
                    </div>
                    <div className="bg-white dark:bg-slate-800 rounded shadow p-3 text-center">
                        <p className="text-xs text-gray-500 dark:text-gray-400">Expenses</p>
                        <p className="text-sm font-bold text-red-600 dark:text-red-400">${totalExpenses.toFixed(0)}</p>
                    </div>
                    <div className="bg-white dark:bg-slate-800 rounded shadow p-3 text-center">
                        <p className="text-xs text-gray-500 dark:text-gray-400">Balance</p>
                        <p className={`text-sm font-bold ${balance >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>${balance.toFixed(0)}</p>
                    </div>
                </div>
            </div>
            ...
            {transactions.length === 0 && (
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
                    <div className="bg-blue-50 border border-blue-200 rounded p-4">
                        <p className="text-sm text-blue-800 mb-2">👋 Get started quickly with sample data:</p>
                        <button onClick={handleSeedDummyData} disabled={seedingData}
            ...
            <div className="lg:hidden max-w-7xl mx-auto px-4 pb-2">
                <button onClick={() => setFiltersOpen((v) => !v)}
                    ...
                    {hasActiveFilters && <span className="ml-1 bg-blue-600 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">!</span>}
                </button>
                {filtersOpen && (
                    ...
                    <div className="lg:col-span-3">
                        <div className="bg-white dark:bg-slate-800 rounded shadow p-4">
                            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                                Transactions
                                <span className="text-sm font-normal text-gray-500 dark:text-gray-400 ml-2">({filteredTransactions.length})</span>
                            </h3>
                            {loading ? (
                    ...
                    <div className="hidden lg:block lg:col-span-1">
                        <div className="bg-white dark:bg-slate-800 p-4 rounded shadow sticky top-20 space-y-4">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">${totalIncome.toFixed(2)}</p>
                            </div>
                            <div>
                                <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold">Total Expenses</p>
                                <p className="text-2xl font-bold text-red-600 dark:text-red-400 mt-1">${totalExpenses.toFixed(2)}</p>
                            </div>
                            <div className="pt-3 border-t dark:border-slate-700">
                                <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold">Balance</p>
                                <p className={`text-3xl font-bold mt-1 ${balance >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                                    ${balance.toFixed(2)}
                                </p>
                            </div>
                        </div>
